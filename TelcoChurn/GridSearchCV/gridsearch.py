...
class tune_models(luigi.Task):
    ...
    def run(self):
        ...
        def get_features_using_boruta(estimator,features,target):
            boruta_feature_selector = BorutaPy(estimator, n_estimators='auto', verbose=2, random_state=4242, max_iter=50,perc=90)
            boruta_feature_selector.fit(features.values, target.values)
            final_features = list()
            indexes = np.where(boruta_feature_selector.support_ == True)
            for x in np.nditer(indexes):
                final_features.append(features.columns[x])
            return final_features



        print('Running Boruta for Feature Selection')
        ranfor = RandomForestClassifier()
        final_features = get_features_using_boruta(ranfor,features,target)
        borutafeatures = preprocessed_churn[final_features].values
        borutatarget = preprocessed_churn['churn'].values

        def grid_search_cv_estimators(estimator, estimatorfeatures, estimatortarget):
            lr_param_grid = {
                'C': [.0001, .001, .1, 1, 10],
                'penalty': ['l1', 'l2'],
            }
            ...
            if estimator == logreg:
                ...
                lr_grid_search.fit(estimatorfeatures, estimatortarget)
                print('Best parameteres obtained for Logistic Regression are',lr_grid_search.best_params_)
            elif estimator == ranfor:
                rf_grid_search = GridSearchCV(ranfor, rf_param_grid, cv=3, return_train_score=True, n_jobs=-1)
                rf_grid_search.fit(estimatorfeatures, estimatortarget)
                print('Best parameteres obtained for Random Forest Regression are ', rf_grid_search.best_params_)
            elif estimator == xgb:
                xgb_grid_search = GridSearchCV(xgb, xgb_param_grid, cv=3, return_train_score=True, n_jobs=-1)
                xgb_grid_search.fit(estimatorfeatures, estimatortarget)
                print('Best parameteres obtained for XGBoost are ', xgb_grid_search.best_params_)

        logreg = LogisticRegression()
        grid_search_cv_estimators(logreg, borutafeatures,borutatarget)
        ranfor = RandomForestClassifier()
        grid_search_cv_estimators(ranfor, borutafeatures, borutatarget)
        xgb = XGBClassifier()
        grid_search_cv_estimators(xgb, borutafeatures, borutatarget)

        X_train, X_test, y_train, y_test = train_test_split(borutafeatures, borutatarget, random_state=0)

        print('Trying to tune Logistic Regression with reduced features')
        # Initial Model
        logreg = LogisticRegression(C = 0.1, penalty='l2', fit_intercept=False)
        logreg.fit(X_train, y_train)
        model_metrics(logreg, 'Tuned Logistic Regression', X_train, X_test, y_train, y_test)
        print('Performed Logistic Regression after Grid Search CV')
        print('\n')



        print('Trying to tune Random Forest Regressor with reduced features')
        ranfor = RandomForestClassifier(n_estimators=300, criterion = 'gini', max_depth = 10, min_samples_split = 30, min_samples_leaf = 10)
        ranfor.fit(X_train, y_train)
        model_metrics(ranfor, 'Tuned Random Forest', X_train, X_test, y_train, y_test)
        print('\n')

        print('Trying to tune XGBoost with reduced features')
        xgb = XGBClassifier(learning_rate = 0.1, max_depth= 6,min_child_weight=100,n_estimators=50)
        xgb.fit(X_train, y_train)
        model_metrics(xgb, 'Tuned XGBoost', X_train, X_test, y_train, y_test)
        print("Performed XGBoost")
        print('\n')

        print('Pickling the Model')
        # Pickle the model
        finalized_model = open("RFFinalModel.pkl", "wb")
        pickle.dump(ranfor, finalized_model)
    ...
    def output(self):
        return luigi.LocalTarget('tunedmodelevaluation.csv')
...
class upload_to_s3(luigi.Task):
    ...
    def run(self):
        ...
        try:
            ...
            bucket_name = 'tunedmodelevaluatedtelcochurn' + str(st).replace(" ", "").replace("-", "").replace(":", "").replace(".","")
            ...
            print('The path is', tune_models().output().path)
            s3.upload_file(tune_models().output().path, bucket_name, 'tunedmodelevaluation.csv')
